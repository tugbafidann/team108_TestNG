...
public class C06_SoftAssert {
    ...
    public void webappTesti(){
        ...
        String actualSecit=select.getFirstSelectedOption().getText();
        softAssert.assertEquals(actualSecit,expectedSecim,"dropdawnda eurozone secili degil");
        //10. soft assert kullanarak DropDown listesinin su secenekleri oldugunu test edin
            //"Select One", "Australia (dollar)", "Canada (dollar)","Switzerland (franc)","China
            //(yuan)","Denmark (krone)","Eurozone (euro)","Great Britain (pound)","Hong Kong
            //(dollar)","Japan (yen)","Mexico (peso)","Norway (krone)","New Zealand
        ...
        List<String> actualOptionStringList= ReusableMethods.stringListeCevir(optionsElementListesi);
